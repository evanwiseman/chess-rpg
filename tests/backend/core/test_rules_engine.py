...
from src.backend.actions import Attack, Spell, Action, Move
from src.backend.board import Board
from src.backend.core import RulesEngine, Player
from src.backend.entities.pieces import Pawn, Bishop, Knight, Rook, Queen
from src.backend.foundation import ActionType, MoveType, Team
if TYPE_CHECKING:
...
class TestRulesActions(RulesTestCase):
    def test_attack_and_spell_single_target(self):
        ...
        self.assertIn(Action(bishop, attack, pawn, ActionType.ATTACK), actions)
        self.assertIn(Action(bishop, spell, pawn, ActionType.SPELL), actions)
    ...
    def test_attack_multi_target(self):
        ...
        actions = RulesEngine.get_valid_actions(bishop, self.board)
        self.assertIn(
            Action(bishop, attack1, pawn1, ActionType.ATTACK), actions
        )
        self.assertNotIn(
            Action(bishop, attack1, pawn2, ActionType.SPELL), actions
        )
        self.assertIn(
            Action(bishop, attack2, pawn2, ActionType.ATTACK), actions
        )
    ...
    def test_spell_multi_target(self):
        ...
        actions = RulesEngine.get_valid_actions(bishop, self.board)
        self.assertIn(Action(bishop, spell, pawn1, ActionType.SPELL), actions)
        self.assertIn(Action(bishop, spell, pawn2, ActionType.SPELL), actions)
